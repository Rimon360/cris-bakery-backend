async function main(imageOutputDir, options) {
    ...
    function processData(excelData, startDate, endDate, selectedProducts) {
        ...
        excelData.forEach((row, index) => {
            ...
            dailyData[dateKey][productName] += quantity;
            matchedRows++;
            totalQuantity += quantity;

            if (matchedRows <= 5) {
                //console.log(`✓ Row ${index}: ${productName} on ${dateKey} qty=${quantity}`);
            }
        });
        ...
        const configuration = {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: `Products Qty (${startDate} to ${endDate})`,
                        font: {
                            size: 26,
                            weight: 'bold'
                        }
    ...
    try {
        //console.log('=== STARTING PRODUCTS QTY CHART ===\n');
